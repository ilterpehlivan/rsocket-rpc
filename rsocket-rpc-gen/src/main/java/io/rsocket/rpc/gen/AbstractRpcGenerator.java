...
import io.rsocket.rpc.RSocketOptions;
import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;
import java.util.stream.Collectors;

public abstract class AbstractRpcGenerator extends Generator {
  private static final int SERVICE_NUMBER_OF_PATHS = 2;
  private static final int METHOD_NUMBER_OF_PATHS = 5;
  ...
  private List<ServiceContext> findServices(
    ...
    protos.forEach(
        fileProto -> {
          for (int serviceNumber = 0;
              serviceNumber < fileProto.getServiceCount();
              serviceNumber++) {
  ...
  private ServiceContext buildServiceContext(
    ...
    List<Location> allLocationsForService =
        locations.stream()
            .filter(
                location ->
    ...
    for (int methodNumber = 0; methodNumber < serviceProto.getMethodCount(); methodNumber++) {
      MethodContext methodContext =
          buildMethodContext(
              serviceProto.getMethod(methodNumber), typeMap, locations, methodNumber);
  ...
  private MethodContext buildMethodContext(
    ...
    if (!methodProto.getClientStreaming() && methodProto.getServerStreaming()) {
      methodContext.reactiveCallsMethodName = "requestStream";
      methodContext.grpcCallsMethodName = "asyncServerStreamingCall";
    }
    if (methodProto.getClientStreaming() && !methodProto.getServerStreaming()) {
      methodContext.reactiveCallsMethodName = "streamingRequestSingleResponse";
      methodContext.grpcCallsMethodName = "asyncClientStreamingCall";
    }
    if (methodProto.getClientStreaming() && methodProto.getServerStreaming()) {
      methodContext.reactiveCallsMethodName = "streamingRequestAndResponse";
      methodContext.grpcCallsMethodName = "asyncBidiStreamingCall";
    }
    if (methodContext.isFireAndForget) {
      methodContext.reactiveCallsMethodName = "fireAndForget";
      methodContext.grpcCallsMethodName = "asyncUnaryCall";
    }
